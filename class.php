...
class JumpOut
{
    public 
        $settings;
    private
        $settings_default, $jo_url, $popupfiles_domain,
        $version = '3.2.0';

    function JumpOut($is_admin = FALSE)
    {
        ...
        if ($is_admin && time() - 60 * 60 * 24 > $this->settings['last_sync'] && NULL !== $this->settings['session_token']) {
            $this->syncScripts();
        }
    }
    ...
    private function loadSettings()
    {
        ...
        if (FALSE !== strpos($_SERVER['SERVER_NAME'], 'makedreamprofits.my')) {
            ...
            if ('ru' == $this->settings['lang'] || !isset($this->settings['lang'])) {
                $this->jo_url = 'http://jumpout.makedreamprofits.ru/';
                $this->popupfiles_domain = 'files.jumpoutpopup.ru';

            } elseif ('en' == $this->settings['lang']) {
        ...
        //if (defined('JSON_ERROR_NONE') || json_last_error() == JSON_ERROR_NONE) {
            ...
            if ('error' == $result->status) {
                ...
                if (property_exists($result, 'user')) {
                    $this->settings['lang'] = $result->user->lang;

                    if (property_exists($result->user, 'onecode')) {
                        $this->settings['onecode'] = (int)$result->user->onecode;
                    }
                    if (property_exists($result->user, 'main_loader_name')) {
                        $this->settings['main_loader_name'] = $result->user->main_loader_name;
                    }
                }
    ...
    public function frontendRun($content) {
        ...
        if (FALSE === strpos($content, '<!-- jo inserted -->')) {
            ...
            } elseif (isset($this->settings['onecode']) && $this->settings['onecode']) {
                if (FALSE !== strpos($content, '</body>')) {
                    // добавляем код в тело
                    $content = str_replace('</body>', $code . '</body>', $content);
                } else {
                    // ну а если шапки нет - в шапку
    ...
    private function getCodeToPaste() {
        ...
        if (!empty($settings['code'])) {
            return stripslashes($settings['code']);
        }
        ...
        if (isset($settings['list']) && 0 != count($settings['list']) && ((!isset($settings['onecode'])) || (isset($settings['onecode']) && 0 == $settings['onecode']))) {
            foreach ($settings['list'] as $key => $item) if (in_array($item['id'], $settings['activated'])) {
                ...
                if (FALSE !== array_search('', $item['work_on_page'])) {
                    ...
                    $restricted_rules_exists = FALSE;

                    foreach ($item['work_on_page'] as $work_on_page) {

                        $res = $this->pageMatchUrl($REQUEST_URI, $this->prepageWorkOnPageRule($work_on_page));

                        if ($res[0] && !$res[1]) {
                            /* if its not an exception rule */
                            $work_allowed = TRUE;
                        } else if ($res[0] && $res[1]) {
                            /* if even one !restricted rule has met - turn off script */ 
                            $restricted_rules_applied = TRUE;
                        }

                        if ($res[1]) {
                            $restricted_rules_exists = TRUE;
                        }

                        //if ('~/testtest[0-9]?/' == $work_on_page)
                        //$code .= $REQUEST_URI . ':' . $work_on_page . ':' . $this->prepageWorkOnPageRule($work_on_page);
                    }


                    /* if no allowing rules and no restricted rules has met(BUT restricted rules exists) - allowing anyway */
                    if (!isset($work_allowed) && $restricted_rules_exists && !$restricted_rules_applied) {
                        $work_allowed = TRUE;
                        $restricted_rules_applied = FALSE;
                    }

                    if (!$work_allowed || $restricted_rules_applied) {
                        $display_code = FALSE;
                    } else {
                        $display_code = TRUE;
                    }
        ...
        } elseif (isset($settings['onecode']) && $settings['onecode']) {
            $code .= $this->generateOneCode($settings['main_loader_name']);
        }
        ...
        if (isset($settings['magic_begins']) && isset($settings['magic_begins']['enabled']) && TRUE === $settings['magic_begins']['enabled'])
        {
            ...
            if (isset($settings['magic_begins']['async']) && TRUE === $settings['magic_begins']['async']) {
                $code .= '(function(i, s, o, g, a, m) {a = s.createElement(o),m = s.getElementsByTagName(o)[0];a.async = 1;a.src = g;m.parentNode.insertBefore(a, m)})(window, document, \'script\', \'//files.jumpoutpopup.ru/wmb.js\');</script>';
             
            } else {
                $code .= '</script><script src="//files.jumpoutpopup.ru/wmb.js" type="text/javascript" charset="utf-8"></script>';
            }
    ...
    // the same function in the wp plugin and in jo model
    private function prepageWorkOnPageRule($page) {
        $rules = array('!', '~');
        ...
        // remove slash from the end (if * at the end to check / before it)
        $indent = ('*' == substr($page, -1)) ? -1 : 0;
        $clean_url = str_replace('*', '', $page); // for path such as: /*
        foreach ($rules as $rule) {
            if (in_array($clean_url[0], $rules)) {
                $clean_url = substr($clean_url, 1);
            }
        }
        if ('/' != $clean_url && '/' == substr($page, (-1 + $indent), 1)) {
            $page = substr($page, 0, (-1 + $indent));
            if (-1 == $indent) $page .= '*';
        }
        // if exception and then regex - flip sides !~ => ~!
        $page = str_replace('!~', '~!', $page);
        // if not regex(starts with ~) and has * - replace on regex (add ~ and * => .*)

        if ('~' != substr($page, 0, 1) && FALSE !== strpos($page, '*')) {
            $page = '~' . str_replace('*', '.*', $page);
        }
        // if regex - add ^ and $ and replace / => \/
        if ('~' == substr($page, 0, 1)) {
            $page = str_replace('/', '\/', $page) . '$';
            $indent = 1; // ~
            if (FALSE !== strpos(substr($page, 0, 4), '!')) {
                $indent++; // !
            }
            $page = substr_replace($page, '^', $indent, 0);
        }
    ...
    private function generateCode($id, $uid) {
        ...
        $code = '<!--' . __('Начало кода "JumpOut"', 'jumpout') . ' (id:' . $id . ')--><script type="text/javascript">(function(d,w){n=d.getElementsByTagName("script")[0],s=d.createElement("script"),f=function(){n.parentNode.insertBefore(s,n);};s.type="text/javascript";s.async=true;qs=document.location.search.split("+").join(" ");re=/[?&]?([^=]+)=([^&]*)/g;while(tokens=re.exec(qs))
            if("email"===decodeURIComponent(tokens[1]))m=decodeURIComponent(tokens[2]);s.src="//' . $this->popupfiles_domain . '/' . $uid . '-' . $js_file . '.js";if("[object Opera]"===w.opera)d.addEventListener("DomContentLoaded",f,false);else f();})(document,window);</script><!--' . __('Конец кода "JumpOut"', 'jumpout') . ' (id:' . $id . ')-->';

        return $code;
    }

    private function generateOneCode($main_loader_name) {

        return '<script async src="//' . $this->popupfiles_domain . '/' . $main_loader_name . '.js"></script>';
    ...
    function pages() {
    	...
    	switch ($_GET['action']) {
            ...
            case 'save_code':
                ...
                $this->settings['code'] = (!empty($_POST['code'])) ? stripslashes((string)$_POST['code']) : NULL;
                $this->saveSettings();
            ...
            case 'get_session_key':
                ...
                $this->pageRender('Добавление', 'session_token_error', $data);
            break;
            ...
            */
    		...
    		case 'list':
                ...
                if (isset($settings['onecode']) && $settings['onecode']) {
                    $this->pageRender('', 'onecode', $settings);
                } else {
            ...
            case 'debug':
                echo __('Информация для дебага:');
                echo ' server ip: ' . $_SERVER['SERVER_ADDR'];
                echo ' curl: ' . ((function_exists('curl_version')) ? 'yes' : 'no');
                echo ' allow_url_fopen: ' . (('1' == ini_get('allow_url_fopen')) ? 'yes' : 'no');
                $salt = md5(time());
                echo ' loading http://jumpout.makedreamprofits.ru/termsofuse/?' . $salt . ' result: ';
                $res = $this->loadWebContent('http://jumpout.makedreamprofits.ru/termsofuse/?' . $salt);
                if (is_string($res)) {
                    echo 'string' . strlen($res);
                } else {
                    var_dump($res);
                }
            break;
    ...
    private function pageMatchUrl($current_url, $work_on_page) {


        $regex = FALSE;
        $exception = FALSE;

        $work_on_page = mb_strtolower($work_on_page, 'UTF-8');

        // checking for regex
        if (0 === strpos($work_on_page, '~')) {
            $regex = TRUE;
            $work_on_page = substr($work_on_page, 1);
        }
        ...
        /* cutting the last slash */
        if ('/' != $current_url)
            $current_url = ('/' == substr($current_url, -1)) ? substr($current_url, 0, -1) : $current_url;

        if ($regex) {
            $res = $this->wopRegex($current_url, $work_on_page);
        } else {
            $res = ($work_on_page == $current_url);
        }

        // if exception - returning mirrored results
        return ($exception) ? array($res, TRUE) : array($res, FALSE);
    }



    private function wopRegex($current_url, $work_on_page) {
        //console.log('RegExp', work_on_page, 'current_url', current_url);
        preg_match('/' . $work_on_page . '/i', $current_url, $matches);
